...
async def adminChat(user_widget_config: ProjectCreate, db: Session = Depends(get_db)):
    ...
    if not user_widget_config.url:
        return {"status": "error", "message": "Ошибка! Ссылка не найдена"}
    ...
    try:
        ...
        if parse_data:
            ...
            return {"status": "error", "message": "Ошибка при парсинге сайта"}
    ...
    return {
        "status": "success",
        "script_code": f'<script src="{FRONTEND_URL}/widget.js" data-id="{new_id}"></script>',
        "id": new_id,
    }
